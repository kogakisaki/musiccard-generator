...
interface WelcomeModalProps {
  isOpen: boolean;
  onClose: () => void;
  isMobileDevice: boolean; // Thêm prop mới
}

export const WelcomeModal: React.FC<WelcomeModalProps> = ({ isOpen, onClose, isMobileDevice }) => {
  ...
  return (
    ...
    >
      ...
      >
        {isMobileDevice ? (
          <>
            <h2 className="text-2xl font-bold mb-4 text-red-500">Cảnh báo!</h2>
            <p className="text-gray-300 mb-6">
              Trình tạo thẻ nhạc này được tối ưu hóa cho máy tính để bàn.
              Để có trải nghiệm tốt nhất và tránh các vấn đề về hiển thị,
              vui lòng truy cập trang web này từ máy tính hoặc chuyển sang chế độ trang web dành cho máy tính trên thiết bị di động của bạn.
            </p>
          </>
        ) : (
          <>
        ...
        >
          {isMobileDevice ? 'Tôi đã hiểu' : 'Bắt đầu nào!'}
        </button>
      </div>
    </div>
  );
